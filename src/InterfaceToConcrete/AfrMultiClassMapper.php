...
class AfrMultiClassMapper
{
	...
	protected static function getMaxDirTs(string $sDirPath, int $iMaxTimestamp = 0): int
	{
		...
		if (substr($sDirPath, -1) === '*') { //fix composer wildcards
			return static::getMaxDirTs(rtrim($sDirPath, DIRECTORY_SEPARATOR . '*'), $iMaxTimestamp);
		}

		$dh = opendir($sDirPath);
		if ($dh === false) {
			throw new AfrInterfaceToConcreteException(
				'Unable to open directory "' . $sDirPath . '" for reading in ' . __CLASS__ . '@' . __METHOD__ . '(' . implode(', ', func_get_args()) . ')'
			);
		}
