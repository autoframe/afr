...
class AfrToConcreteStrategiesClass implements AfrToConcreteStrategiesInterface
{
	...
	// PRIORITY
	public const PriorityRuleFail = 'fail';
	public const PriorityRuleOnlyClosureFn = 'onlyClosureFn';
	public const PriorityRuleNeverFail = 'neverFail';
	public const PriorityRuleNeverFailWithoutWarnings = 'neverFailWithoutWarnings';
	...
	protected array $aPriorityRules = [
		...
		self::PriorityRuleNeverFailWithoutWarnings => [
			self::StrategyClosureFn, //multiple
			self::StrategyContextBound, //single
			self::StrategyContextNamespaceFilterArr, //multiple
			self::StrategyContextHttpRequestUriRegex, //multiple
	...
	{
		if (empty(static::$oLatestInstance)) {
			static::$oLatestInstance = new static();
		}
		return static::$oLatestInstance;
	}
	...
	{
		return static::$oLatestInstance = $oLatestInstance;
	}
	...
	{
		if ($oAfrInterfaceToConcreteInterface === null) {
			$oAfrInterfaceToConcreteInterface = AfrInterfaceToConcreteClass::getLatestInstance();
		}
		return $this->resolveMap(
			$oAfrInterfaceToConcreteInterface->getClassInterfaceToConcrete($notConcreteFQCN),
			$notConcreteFQCN,
			$bCache
		);
	}
	...
	public function resolveMap(array $aMappings, string $notConcreteFQCN, bool $bCache = true): string
	{
		...
		foreach ($aMappings as $implementingClass => $mInstantiable) {
			//default values are bool, but known overwrites are allowed: 0,1,2
			if (
				$mInstantiable === true ||
				$mInstantiable === 1 || $mInstantiable === '1' ||
				$mInstantiable === 2 || $mInstantiable === '2'
			) {
				//1:instantiable; 2:singleton pretested
				$aMappings[$implementingClass] = (string)$mInstantiable;
				continue;
			} elseif ($mInstantiable === false) {
