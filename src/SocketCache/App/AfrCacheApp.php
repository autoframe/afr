...
class AfrCacheApp extends AfrSingletonArrAbstractClass // extends Autoframe\Core\SocketCache\LaravelPort\Contracts\Foundation\Application
{
    /**
     * @var \Closure|null
     */
    ...
    ): self
    {
        ...
        if (
            ...
            $aData['extend'] = function ($oApp, $aConfig) {
                return $this->repository(
                    new \Autoframe\Core\SocketCache\LaravelPort\Cache\ArrayStore(
    ...
    ): self
    {
        ...
        if (empty($this->aArrayAccessData['files'])) {
            $this->aArrayAccessData['files'] = new \Autoframe\Core\SocketCache\LaravelPort\Filesystem\Filesystem();
        }
        ...
        if (
            ...
            $aData['extend'] = function ($oApp, $aConfig) {
                return $this->repository(
                    new \Autoframe\Core\SocketCache\LaravelPort\Cache\FileStore(
    ...
    */
        ...
        if (empty($this->aArrayAccessData['memcached.connector'])) {
            $this->aArrayAccessData['memcached.connector'] =
                new \Autoframe\Core\SocketCache\LaravelPort\Cache\MemcachedConnector(); //singleton preferred
        }
        ...
        if ( //second custom connection
            ...
            $aData['extend'] = function ($oApp, $aConfig) {
                return $this->repository(
                    new \Autoframe\Core\SocketCache\LaravelPort\Cache\MemcachedStore(
    ...
    ): self
    {
        ...
        if (empty($this->aArrayAccessData['redis'])) {
            ...
            $this->aArrayAccessData['redis'] =
                new \Autoframe\Core\SocketCache\LaravelPort\Redis\RedisManager(
                    $this,
                    $sRedisDriver,
                    $aDbRedisCfg
                );
        ...
        if ( //TODO: test REDIS alt config, because it may not work...
            ...
            $aData['extend'] = function ($oApp, $aConfig) {
                ...
                $store = new \Autoframe\Core\SocketCache\LaravelPort\Cache\RedisStore(
                    $oApp['redis'],
                    $this->getPrefix($aConfig),
                    $connection
                );
