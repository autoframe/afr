...
namespace Autoframe\Core\SocketCache\LaravelPort\Redis\Connectors;

use Autoframe\Core\SocketCache\LaravelPort\Contracts\Redis\Connector;
use Autoframe\Core\SocketCache\LaravelPort\Redis\Connections\PhpRedisClusterConnection;
use Autoframe\Core\SocketCache\LaravelPort\Redis\Connections\PhpRedisConnection;
...
class PhpRedisConnector implements Connector
{
     ...
     * @param  array  $config
     * @param  array  $options
     * @return \Autoframe\Core\SocketCache\LaravelPort\Redis\Connections\PhpRedisConnection
     */
     ...
     * @param  array  $options
     * @return \Autoframe\Core\SocketCache\LaravelPort\Redis\Connections\PhpRedisClusterConnection
     */
    ...
    {
        return Tap::tap(new Redis, function ($client) use ($config) {
    ...
    protected function createRedisClusterInstance(array $servers, array $options)
    {
        ...
        return Tap::tap(new RedisCluster(...$parameters), function ($client) use ($options) {
    ...
    protected function formatHost(array $options)
    {
        if (isset($options['scheme'])) {
            return $this->start($options['host'], "{$options['scheme']}://");
        }

        return $options['host'];
    }

    protected function start($value, $prefix)
    {
